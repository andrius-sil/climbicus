...
def test_view_logbook(client, auth_headers_user1):
    ...
    resp = client.get("/user_route_log/logbooks/view", data=json.dumps(data), content_type="application/json", headers=auth_headers_user1)
...
def test_add_to_logbook(client, app, auth_headers_user1):
    ...
    resp = client.post("/user_route_log/logbooks/add", data=json.dumps(data), content_type="application/json", headers=auth_headers_user1)
    assert resp.status_code == 200
